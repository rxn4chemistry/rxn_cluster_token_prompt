...
from rxn_cluster_token_prompt.clustering.clusterer import (
    Clusterer,
    ClustererFitter,
    inspect_clusters,
)
...
@click.command()
@click.option(
    "--clusterer_pkl", type=str, required=True, help="Where to store the clusterer."
)
@click.option(
    "--pca_components",
    type=int,
...
@click.option(
    ...
    type=int,
    default=10,
    help="The number of clusters to use, default is 10.",
)
def main(clusterer_pkl: str, pca_components: int, n_clusters: int):
    ...
    all_fps = fps
    logger.info(f"Merged, shuffled: {len(all_fps)}")
